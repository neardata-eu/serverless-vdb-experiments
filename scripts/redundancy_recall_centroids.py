...
for p in [16, 32, 64, 128]:
    ...
    ax.set_xticks(x)
    ax.set_xlabel("Number of Partitions Searched")
    ax.set_ylabel("Recall")
    ax.set_title(f"Recall ({p} partitions)")
    ...
    for i, r in enumerate([0, 1, 2, 5]):
        mean_recalls = [np.mean(results[p][r]["recalls"][n]) for n in num_centroids_searches]
        ax_inset.errorbar(x, mean_recalls, 
                          linestyle=line_styles[i % len(line_styles)],
                          markersize=3,
                          linewidth=0.8,
                          capsize=2,
    ...
    plt.savefig(f'../plots/replication_analysis/replication_analysis_recall_{p}_partitions.pdf', dpi=300, bbox_inches='tight')
